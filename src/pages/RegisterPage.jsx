    ...
    <>
      <div className="flex min-h-screen items-center justify-center bg-slate-100 p-4">
        <form onSubmit={handleRegister} className="h-auto w-xl p-[25px] rounded-xl bg-[#B2BEB5] shadow-md">
          ...
          <input id="accname" type="text" placeholder="ชื่อผู้ใช้" value={accname} onChange={(e) => setAccname(e.target.value)} onInvalid={(e) => e.target.setCustomValidity('กรุณากรอกชื่อ')} onInput={(e) => e.target.setCustomValidity('')} className="w-full h-10 p-2 mb-4 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 bg-white" required/>

          <input id="username" type="text" placeholder="ชื่อบัญชี" value={username} onChange={(e) => setUsername(e.target.value)} onInvalid={(e) => e.target.setCustomValidity('กรุณากรอกบัญชี')} onInput={(e) => e.target.setCustomValidity('')} className="w-full h-10 p-2 mb-4 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 bg-white" required/>
